...
class GamePiece{
    public:
        // Constructor initialized once an object of the class is created with mPieceColor()
        ...
        bool IsLegalMove(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[8][8]){
            ...
            // qpDest is where the piece will be moved to
            // qpDest == 0 denotes the destination square will be unoccupied
    ...
    private:
        ...
        bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[8][8]){
            ...
            if(qpDest == 0){
                ...
                if(iSrcCol == iDestCol){
                    // If it's white playing it will move one row forward
                    if(GetColor() == 'W'){
    ...
    private:
        ...
        bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[8][8]){
            if((iDestCol - iSrcCol == iDestRow - iSrcRow) || (iDestCol - iSrcCol == iSrcRow - iDestRow)){
                // Make sure that all intervening sqaures are empty
                int iRowOffset = (iDestRow - iSrcRow > 0) ? 1 : -1;
                int iColOffset = (iDestCol - iSrcCol > 0) ? 1 : -1;
                int iCheckRow;
                int iCheckCol;
                for(iCheckRow = iSrcRow + iRowOffset,iCheckCol = iSrcCol + iColOffset; iCheckRow != iDestRow; iCheckRow = iCheckRow + iRowOffset,iCheckCol = iSrcCol + iColOffset){
    ...
    private:
        ...
        bool AreSquaresLegal(int iSrcRow, int iSrcCol, int iDestRow, int iDestCol, GamePiece* GameBoard[8][8]){
            if(iSrcRow == iDestRow){
                ...
                // Make sure that all interverning squares are empty
                int iRowOffset = (iDestRow - iSrcRow > 0) ? 1 : -1;
...
class KingPiece : public GamePiece{
    ...
    private:
        ...
        bool AreSqauresLegal(int iSrcCol, int iSrcRow, int iDestRow, int iDestCol, GamePiece* GameBoard[8][8]){
    ...
    public:
        ...
        void Print(){
            ...
            for(int iRow = 0; iRow < 8*kiSquareHeight; ++iRow){
                ...
                // Print side bordering with numbering
                if(iRow % 3 == 1){
        ...
        bool IsInCheck(char PieceColor){
            ...
            // Run through the opponent's pieces and see if they can take the king
            for(int iRow = 0;iRow < 8; ++iRow){
                for(int iCol = 0;iCol < 8; ++iCol){
                    if(MainGameBoard[iRow][iCol] != 0){
                        if(MainGameBoard[iRow][iCol]->GetColor() != PieceColor){
...
class ChessBoard{
    public:
        ...
        void GetNextMove(GamePiece* GameBoard[8][8]){
            ...
            do{
                system("Clear");
                cout<<endl<<endl<<" Welcome to the Game of Intellectuals"<<endl<<endl<<endl;
